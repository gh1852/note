...
### 使用场景
- **共享分支撤销**：在团队协作的共享分支上安全撤销提交，保留完整历史记录
- **本地提交修正**：撤销本地未推送的提交（可使用reset）
- **批量撤销**：一次性撤销多个连续或非连续的提交
- **合并提交撤销**：撤销merge提交并指定保留哪个父分支
- **远程提交撤销**：撤销已推送到远程仓库的提交
- **冲突解决**：处理revert过程中可能出现的冲突情况
